...
export async function POST() {
  log("Projectors ON requested");
  const config = getConfig();

  const results = await Promise.allSettled(
    config.PROJECTORS.map(async (p) => {
      const result = await sendPJLink(p.ip, "%1POWR 1");
  ...
  log(`Projectors ON results: ${JSON.stringify(response)}`);
  return NextResponse.json({ action: "on", projectors: response });
}
